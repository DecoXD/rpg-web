...
const AuthForm = ({inputs,onSubmit,buttonLabel,title,linkHref,linkLabel,linkName}:AuthFormProps) => {
  return (
     ...
     <h1 className='text-3xl font-medieval font-bold'>{title}</h1>
      <form onSubmit={onSubmit} className='flex flex-col gap-4 w-full max-w-md'>
        ...
        inputs.map((input,index)=>(
          <Input key = {`${input.label}-${index}}`} {...input} required={true}/>
        ))}
        <Button className='bg-amber-300 hover:bg-amber-400 text-black font-medieval font-medium'>{buttonLabel}</Button>
      </form>
      <p className='text-sm text-gray-500 font-bold font-medieval'>{linkLabel}<Link className='text-black/80  ' to={linkHref}>{linkName}</Link></p>
    </div>
  )
}
